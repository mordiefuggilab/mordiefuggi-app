...
// --- TRADUZIONE DINAMICA VIA API (nessun dizionario statico) ---
// Chiama /api/translate che usa MyMemory (gratuito, senza chiave)
// Funziona automaticamente per qualsiasi piatto aggiunto su Supabase
...
export default function MordieFuggiApp() {
  ...
  // --- LINGUA MENU ---
  const [menuLang, setMenuLang] = useState('it'); // 'it' | 'en' | 'fr' | 'es'
  const [translations, setTranslations] = useState({}); // { en: {id:nome}, fr: {id:nome}, es: {id:nome} }
  ...
  const switchLang = async (lang) => {
    if (lang === menuLang) { setMenuLang('it'); return; }
    // Already cached for this language?
    const cacheKey = lang;
    if (translations[cacheKey]) { setMenuLang(lang); return; }
    setIsTranslating(true);
    try {
      const piatti = piattiGiorno.filter(p => p.attivo).map(p => ({ id: p.id, nome: p.nome }));
      const res = await fetch('/api/translate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ piatti, lang })
      });
      const text = await res.text();
      if (!text || text.trim() === '') throw new Error('Empty response from translate API');
      const data = JSON.parse(text);
      if (data.translations && data.translations.length > 0) {
        const map = {};
        data.translations.forEach(({ id, nome }) => { map[id] = nome; });
  ...
  const getNome = (p) => {
    if (menuLang === 'it' || !translations[menuLang]) return p.nome;
    return translations[menuLang][p.id] || p.nome;
  };
...
<button onClick={async () => {
  ...
  const { data: esistente } = await supabase.from('storico_menu').select('id').eq('data', oggi).limit(1);
  if (esistente && esistente.length > 0) { alert('📸 Menu di oggi già salvato!'); return; }
  const piattiAttivi = piattiGiorno.filter(p => p.attivo && (p.categoria === 'Primi' || p.categoria === 'Secondi'));
  if (piattiAttivi.length === 0) { alert('Nessun piatto attivo da salvare.'); return; }
  const righe = piattiAttivi.map(p => ({ data: oggi, piatto_id: p.id, piatto_nome: p.nome, categoria: p.categoria }));
  const { error } = await supabase.from('storico_menu').insert(righe);
  if (!error) alert(`✅ Salvati ${righe.length} piatti per oggi!`);
  else alert('Errore salvataggio!');
}} className="w-full mb-8 bg-[#C9A97A] text-white py-4 rounded-[2rem] font-black uppercase italic text-xs flex items-center justify-center gap-3 shadow-xl">
  📸 Salva menu di oggi x le statistiche
</button>
  ...
  <div className="bg-white rounded-[2rem] p-4 shadow-sm">
    <p className="text-[9px] font-black uppercase text-gray-400 mb-3 italic">📦 Piatti in archivio</p>
    <div className="grid grid-cols-5 gap-2">
      {[
        { label: 'Totale', cat: null, color: '#111111' },
        { label: 'Primi', cat: 'Primi', color: '#2E7D32' },
        { label: 'Secondi', cat: 'Secondi', color: '#2E7D32' },
        { label: 'Contorni', cat: 'Contorni', color: '#C9A97A' },
        { label: 'Bevande', cat: 'Bevande', color: '#C9A97A' },
      ].map(({ label, cat, color }) => {
        const count = cat ? piattiGiorno.filter(p => p.categoria === cat).length : piattiGiorno.length;
        return (
          <div key={label} className="text-center">
            <p className="font-black text-xl leading-none" style={{ color }}>{count}</p>
            <p className="text-[8px] font-black text-gray-400 uppercase mt-1">{label}</p>
          </div>
        );
      })}
    </div>
  ...
  <input type="text" placeholder="Cerca piatti..." className="w-full p-4 bg-white rounded-2xl font-bold text-sm shadow-sm outline-none" onChange={(e) => setSearchQuery(e.target.value.toLowerCase())} />
              {piattiGiorno
  .filter(p => (adminCategoryFilter === "Tutti" || p.categoria === adminCategoryFilter))
  .filter(p => p.nome.toLowerCase().includes(searchQuery))
  .sort((a, b) => {
    if (b.attivo !== a.attivo) return b.attivo ? 1 : -1;
    return (CATEGORY_ORDER[a.categoria] || 99) - (CATEGORY_ORDER[b.categoria] || 99);
  })
  .map(p => (
  ...
  if (view === 'menu') return (
    <div className="min-h-screen bg-white p-6 pb-40 text-left">
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => { setView('home'); setMenuLang('it'); }} className="flex items-center gap-2 text-[#606060] font-bold bg-[#EFEFED] px-4 py-2 rounded-full"><ChevronLeft size={20} /> Home</button>
        <div className="flex gap-2">
          {/* UK */}
          <button onClick={() => switchLang('en')} disabled={isTranslating} className={`flex flex-col items-center gap-1 px-2 py-1.5 rounded-2xl border transition-all hover:scale-105 ${menuLang==='en' ? 'border-[#2E7D32] shadow-md bg-green-50' : 'bg-white border-gray-200'} ${isTranslating ? 'opacity-50' : ''}`}>
            <svg width="30" height="18" viewBox="0 0 60 30" xmlns="http://www.w3.org/2000/svg" style={{borderRadius:2,display:'block'}}>
            ...
            <span className={`text-[8px] font-black uppercase ${menuLang==='en' ? 'text-[#2E7D32]' : 'text-gray-400'}`}>EN</span>
          </button>
          {/* France */}
          <button onClick={() => switchLang('fr')} className={`flex flex-col items-center gap-1 px-2 py-1.5 rounded-2xl border transition-all hover:scale-105 ${menuLang==='fr' ? 'border-[#2E7D32] shadow-md bg-green-50' : 'bg-white border-gray-200'}`}>
            <svg width="30" height="18" viewBox="0 0 3 2" xmlns="http://www.w3.org/2000/svg" style={{borderRadius:2,display:'block'}}>
              <rect width="1" height="2" fill="#002395"/>
              <rect x="1" width="1" height="2" fill="#EDEDED"/>
              <rect x="2" width="1" height="2" fill="#ED2939"/>
            </svg>
            <span className={`text-[8px] font-black uppercase ${menuLang==='fr' ? 'text-[#2E7D32]' : 'text-gray-400'}`}>FR</span>
          </button>
          {/* Spain */}
          <button onClick={() => switchLang('es')} className={`flex flex-col items-center gap-1 px-2 py-1.5 rounded-2xl border transition-all hover:scale-105 ${menuLang==='es' ? 'border-[#2E7D32] shadow-md bg-green-50' : 'bg-white border-gray-200'}`}>
            <svg width="30" height="18" viewBox="0 0 3 2" xmlns="http://www.w3.org/2000/svg" style={{borderRadius:2,display:'block'}}>
      ...
      <div className="mb-6">
        ...
        {(() => {
          ...
          const isWeekend = (giorno === 5 && ora.getHours() >= 15) || giorno === 6 || (giorno === 0 && !(ora.getHours() >= 20 && ora.getMinutes() >= 30));
          if (isWeekend) return (
            <p className="text-xs font-bold text-[#C9A97A] uppercase">
              Il nuovo menù sarà disponibile da domenica alle 20:30
            </p>
          );
          const data = new Date();
          if (ora.getHours() >= 20 && ora.getMinutes() >= 30) {
      ...
      {new Date().getHours() >= 13 && new Date().getHours() < 15 && (
        <div className="flex items-center gap-4 bg-[#2E7D32] text-white rounded-[2rem] p-5 mb-6 shadow-xl">
          <Clock size={28} className="text-[#C9A97A] shrink-0" />
          <div>
            <p className="font-black text-sm uppercase italic tracking-wide mb-1">
              Prenotazioni on line sospese
            </p>
            <p className="text-white/80 text-[11px] font-bold leading-relaxed">
              Dopo le 13:00 i piatti potrebbero non essere più disponibili. Pertanto puoi contattarci telefonicamente per avere una disponibilità aggiornata della nostra proposta on line.
            </p>
          </div>
        </div>
      )}
      ...
      {(() => {
        const ora = new Date();
        const giorno = ora.getDay();
        const fasciaPomeriggio = ora.getHours() >= 15 && !(ora.getHours() >= 20 && ora.getMinutes() >= 30);
        const weekend = (giorno === 5 && ora.getHours() >= 15) || giorno === 6 || (giorno === 0 && !(ora.getHours() >= 20 && ora.getMinutes() >= 30));
        if (!fasciaPomeriggio && !weekend) return null;
        return (
          <div className="flex items-center gap-4 bg-[#111111] text-white rounded-[2rem] p-5 mb-6 shadow-xl">
            <Clock size={28} className="text-[#C9A97A] shrink-0" />
            <div>
              <p className="font-black text-sm uppercase italic tracking-wide mb-1">
                Cucina chiusa
              ...
              <p className="text-white/80 text-[11px] font-bold leading-relaxed">
                {weekend ? 'Siamo chiusi nel weekend. Le prenotazioni online riaprono domenica alle 20:30 con il menù di lunedì. A presto!' : 'Le prenotazioni online riaprono alle 20:30 con il menù di domani. A presto!'}
              </p>
            </div>
          </div>
        );
          ...
          <div key={p.id} className={`rounded-[2.5rem] p-5 border shadow-sm mb-4 flex justify-between items-center transition-opacity ${!isCucinaAperta ? 'opacity-60 grayscale' : 'bg-[#F8F9FA] border-[#EFEFED]'}`}>
            <div className="flex gap-4 items-center">
              ...
              <div className="text-left leading-tight">
                <p className="font-black text-sm uppercase">{getNome(p)}</p>
                {p.allergeni && (
            ...
            <div className="flex items-center gap-3 bg-white p-2 rounded-[2rem] border border-gray-100">
              ...
              <button onClick={() => addToCart(p)} disabled={!isCucinaAperta || p.stock - inCart <= 0 || (new Date().getHours() >= 13 && !(new Date().getHours() >= 20 && new Date().getMinutes() >= 30)) || (new Date().getDay() === 5 && new Date().getHours() >= 15) || new Date().getDay() === 6 || (new Date().getDay() === 0 && !(new Date().getHours() >= 20 && new Date().getMinutes() >= 30))} className={`w-10 h-10 flex items-center justify-center text-white rounded-full ${!isCucinaAperta || p.stock - inCart <= 0 || (new Date().getHours() >= 13 && !(new Date().getHours() >= 20 && new Date().getMinutes() >= 30)) || (new Date().getDay() === 5 && new Date().getHours() >= 15) || new Date().getDay() === 6 || (new Date().getDay() === 0 && !(new Date().getHours() >= 20 && new Date().getMinutes() >= 30)) ? 'bg-gray-300' : 'bg-[#2E7D32]'}`}><Plus size={18}/></button>
            </div>
          </div>
        );
      })}
  ...
  <div className="flex gap-2">
    ...
    <button onClick={() => setShowLoginModal(true)} className="p-2 bg-white rounded-full border shadow-sm"><Settings size={20}/></button>
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <button onClick={() => setView('lavora')} className="bg-[#2E7D32] text-white p-6 rounded-[2.5rem] flex items-center gap-6 shadow-xl mt-4"><div className="bg-white/10 p-4 rounded-2xl text-[#C9A97A]"><Briefcase size={32} /></div><div className="font-bold text-xl uppercase italic">Lavora con noi</div></button>
      </div>
    </div>
  );
